...
-- ============================================================
-- 12. PERFIS DE USUÁRIO (autenticação / controle de acesso)
-- ============================================================
...
-- cria automaticamente um perfil (INATIVO, aguardando aprovação) quando alguém cria uma conta
...
-- funções auxiliares (security definer = não geram recursão de RLS)
create or replace function is_active_user()
returns boolean
language sql security definer stable
as $$ select exists(select 1 from perfis where id = auth.uid() and ativo = true); $$;

create or replace function is_admin()
returns boolean
language sql security definer stable
as $$ select exists(select 1 from perfis where id = auth.uid() and papel = 'admin' and ativo = true); $$;

alter table perfis enable row level security;

drop policy if exists "ver_proprio_ou_admin" on perfis;
create policy "ver_proprio_ou_admin" on perfis for select
  using (auth.uid() = id or is_admin());

drop policy if exists "insere_proprio_perfil" on perfis;
create policy "insere_proprio_perfil" on perfis for insert
  with check (auth.uid() = id);

drop policy if exists "admin_atualiza_perfis" on perfis;
create policy "admin_atualiza_perfis" on perfis for update
  using (is_admin()) with check (true);

drop policy if exists "admin_exclui_perfis" on perfis;
create policy "admin_exclui_perfis" on perfis for delete
  using (is_admin());
...
-- ============================================================
-- ROW LEVEL SECURITY
-- O sistema fica hospedado de forma estática (GitHub Pages) e usa a
-- chave "anon" no navegador — isso é normal no Supabase. A proteção
-- real é feita aqui: só usuário autenticado E com perfil "ativo=true"
-- em `perfis` consegue ler ou gravar qualquer dado operacional.
-- Sem login aprovado, a chave anon sozinha não abre porta nenhuma.
-- ============================================================
do $$
declare
  t text;
begin
  ...
  loop
    ...
    execute format('drop policy if exists "usuario_ativo" on %I;', t);
    execute format('create policy "usuario_ativo" on %I for all using (is_active_user()) with check (is_active_user());', t);
  end loop;
end $$;

-- ============================================================
-- BOOTSTRAP DO PRIMEIRO ADMINISTRADOR (leia o README)
-- 1. Crie sua conta na tela de login do sistema (aba "Criar conta").
-- 2. Rode o comando abaixo AQUI no SQL Editor, trocando o e-mail:
--
--    update perfis set papel = 'admin', ativo = true where email = 'seu-email@exemplo.com';
--
-- A partir daí, essa conta é a "master": só ela vê a aba Usuários e
-- pode aprovar, bloquear ou promover qualquer outro colaborador.
-- ============================================================
